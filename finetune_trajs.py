...
def train_epoch(args, epoch, model, data_loader, optimizer, writer, loader_len,orig_diffs=None):
    model.train()
    avg_loss = 0.
    ...
    for iter, data in data_loader:
        ...
        #output = model(input)
        outs = []
        for k in range(0,input.shape[1],8):
            outs.append(model(input[:,k:k+8]))
        output = torch.stack(outs,dim=1).view(*input.shape[:-1])  
        ...
        #reg loss
        diffs = (output[:,1:]-output[:,:-1]).abs()
        reg_avg = (diffs.sum(1)-diffs[:,7])/(diffs.shape[1]-1)
        diff_distance = F.mse_loss(diffs[:,7],reg_avg)
        diff_losses.append(diff_distance.item())
        
        bound_distance = F.relu(diffs.reshape(diffs.shape[0],diffs.shape[1],-1).mean(-1).mean(0) - 0.005).mean()
        #bound_distance = bound_distance**2
        #breakpoint()
        ...
        loss = args.bound_weight*bound_distance + args.diff_weight*diff_distance + args.rec_weight * rec_loss + args.vel_weight * vel_loss + args.acc_weight * acc_loss
        ...
        if iter % args.report_interval == 0:
            logging.info(
                ...
                f'Loss = {loss.item():.4g} Avg Loss = {avg_loss:.4g}, diff loss {np.mean(diff_losses):.4g} '
...
def build_model(args):
    model = Subsampling_Model(
        in_chans=args.num_frames_per_example//2,
        out_chans=args.num_frames_per_example//2,
...
def create_arg_parser():
    ...
    parser.add_argument('--data-path', type=pathlib.Path,
                      default='C:/Users/StrugoEden/Desktop/aug', help='Path to the dataset')
    ...
    parser.add_argument('--checkpoint', type=str, default="/home/tamir.shor/MRI/interleaved_64_128_64/summary/test/best_model.pt",help='Path to an existing checkpoint. Used along with "--resume"')
    ...
    parser.add_argument('--trajectory-learning', default=True, action = "store_true",
    ...
    parser.add_argument('--diff-weight', type=float, default=1, help='weight of the diff in transition loss')
    parser.add_argument('--bound-weight', type=float, default=1, help='weight of the lower bound mean loss')
    ...
    parser.add_argument('--num_frames_per_example', type=int, default=16, help='num frames per example')
