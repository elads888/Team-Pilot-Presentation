...
from MRITramsformer.utils.email import send_email
...
def plot_and_save_image(image, title, epoch, frame):
    # Normalize image to [0, 1]
    image = image - image.min()
    image = image / image.max()
    name = '{}-frame-{}.png'.format(title,frame + 1)
    logger.info(f"Plotting and saving {name}")
    ...
    os.makedirs('test_photos2', exist_ok=True)
    output_dir = f'test_photos2/epoch_{epoch}'
    ...
    plt.title(title)
    plt.axis('off')
    plt.savefig(f'test_photos2/epoch_{epoch}/{name}', bbox_inches='tight')
    plt.close()
...
def train_epoch(args, epoch, model, data_loader, optimizer, writer, loader_len):
    ...
    logger.info(f"started training epoch {epoch}")

    model.train()
    model.subsampling.unet_enc3.eval()
    avg_loss = 0.
    ...
    logger.info(f'Imposing Machine Constraints: a_max={args.a_max}, v_max={args.v_max}')
    ...
    for iter, data in data_loader:
        ...
        #     if iter % 20 < 10:  # First 10 iterations: Train subsampling layer
        #         model.subsampling.requires_grad_(True)
        #         model.reconstruction_model.requires_grad_(False)
        #     else:  # Next 10 iterations: Train reconstruction model
        #         model.subsampling.requires_grad_(False)
        #         model.reconstruction_model.requires_grad_(True)

        acc_loss, vel_loss, rec_loss, psnr_train = None, None, None, None
        acc_loss_iter, vel_loss_iter, rec_loss_iter, psnr_train_iter, loss_iter, avg_loss_iter = [], [], [], [], [], []

        for frame in range(input.size(1)):

            optimizer.zero_grad()
            curr_input_frame = input[:, frame, :, :, :].unsqueeze(1).to(args.device)

            # Print memory usage before the forward pass
            #print("Memory Summary Before Forward Pass:")
            #print(torch.cuda.memory_summary(device=None, abbreviated=False))

            output = model(curr_input_frame).unsqueeze(1)
            if frame == 0:
                current_output = output
            else:
                current_output = torch.cat((current_output, output), dim=1)

            # Print memory usage after the forward pass
            #print("Memory Summary After Forward Pass:")
            #print(torch.cuda.memory_summary(device=None, abbreviated=False))
            ...
            # target loss
            total_current_target = target[:, :frame+1, :, :]
            current_target = target[:, frame, :, :].unsqueeze(1).to(args.device)

            rec_loss = F.mse_loss(output.to(torch.float32), current_target.to(
                torch.float32))
            ...
            # for name, param in model.reconstruction_model.named_parameters():
            #     if param.grad is not None:
            #         print(f"✅ {name} has gradients")
            #     else:
            #         print(f"⚠️ {name} has NO gradients (check computation graph)")

            # for name, param in model.subsampling.named_parameters():
            #     if param.grad is None:
            #         print(f"⚠️ {name} has NO gradient! Check computation graph.")
            ...
            #         else:
            #             print(f"❌ {name} | NO grad (possible disconnect or frozen layer)")
            optimizer.step()
            current_target = current_target.to("cpu")
        ...
        #Apply trajectory freezing - zero grad on all trajectories corresponding to any frame besides the one currently optimized
            ...
            psnr_train, vif_train, fsim_train = get_metrics(current_target, output,args.vif,args.fsim)
            logger.info(f'fsim: {fsim_train} vif: {vif_train} psnr: {psnr_train} loss: {loss.item()}')

            loss_iter.append(loss)
            psnr_train_iter.append(psnr_train)
            avg_loss_iter.append(avg_loss)
            acc_loss_iter.append(acc_loss)
            vel_loss_iter.append(vel_loss)
            rec_loss_iter.append(rec_loss)

        loss = torch.stack(loss_iter).mean(dim=0)
        psnr_train = sum(psnr_train_iter) / len(psnr_train_iter)
        avg_loss = sum(avg_loss_iter) / len(avg_loss_iter)
        acc_loss = sum(acc_loss_iter) / len(acc_loss_iter)
        vel_loss = sum(vel_loss_iter) / len(vel_loss_iter)
        rec_loss = sum(rec_loss_iter) / len(rec_loss_iter)
    ...
    with torch.no_grad():
        if epoch != 0:
            for iter, data in data_loader:
                ...
                target = transforms.complex_abs(input)
                vif_dev_iter, fsim_dev_iter, psnr_dev_iter, loss_iter = [], [], [], []
                for frame in range(input.size(1)):

                    curr_input_frame = input[:, frame, :, :, :].unsqueeze(1)
                    output = model(curr_input_frame).unsqueeze(1)
                    if frame == 0:
                        current_output = output
                    else:
                    ...
                    # target loss
                    total_current_target = target[:, :frame + 1, :, :]
                    current_target = target[:, frame, :, :].unsqueeze(1)
                    loss = F.mse_loss(output.to(torch.float64), current_target.to(
                        torch.float64))
                    psnr_dev, vif_dev, fsim_dev = get_metrics(current_target, output, args.vif, args.fsim)
                    #print(f"Type = {type(fsim_dev)}")
                    loss_iter.append(loss)
                    psnr_dev_iter.append(psnr_dev)
                    fsim_dev_iter.append(fsim_dev)
                    vif_dev_iter.append(vif_dev)
                loss = torch.stack(loss_iter).mean(dim=0)
                psnr_dev = sum(psnr_dev_iter) / len(psnr_dev_iter)
                # fsim_dev = sum(fsim_dev_iter) / len(fsim_dev_iter)
                # vif_dev = sum(vif_dev_iter) / len(vif_dev_iter)
...
def visualize(args, epoch, model, data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(data_loader):
            logger.info(f"Starting iteration {iter}/{len(data_loader)} in visualize")
            ...
            if epoch != 0:
                for frame in range(input.size(1)):
                    logger.info(f"Starting frame {frame}/{input.size(1)}")
                    curr_input_frame = input[:, frame, :, :, :].unsqueeze(1)
                    curr_target_frame = target[:, frame,:, :, :].unsqueeze(1)

                    logger.info(f"Input shape: {curr_input_frame.shape}")
                    output = model(curr_input_frame).unsqueeze(1)
                    logger.info(f"Output shape: {output.shape} (before unsqueeze)")
                    ...
                    corrupted = model.subsampling(curr_input_frame).unsqueeze(2)
                    cor_all = transforms.root_sum_of_squares(corrupted, -1)

                    #plot_and_save_image(current_output[0], 'Reconstruction_Total_Frames', epoch, frame)
                    plot_and_save_image(output[0], f'Reconstruction_For_Frame_{frame + 1}', epoch, frame)
                    plot_and_save_image(curr_target_frame[0], f'Target_For_Frame_{frame + 1}', epoch, frame)
                    plot_and_save_image(corrupted[..., 0][0], 'Corrupted_real', epoch, frame)
                    plot_and_save_image(corrupted[..., 1][0], 'Corrupted_im', epoch, frame)
                    plot_and_save_image(torch.abs(input[0] - output[0]), 'Error', epoch, frame)
                    plot_and_save_image(cor_all[0], 'Corrupted', epoch, frame)

                    save_image(current_output, 'Reconstruction_Total_Frames')
                    save_image(output, f'Reconstruction_For_Frame_{frame + 1}')
                    save_image(corrupted[..., 0], 'Corrupted_real')
                    save_image(corrupted[..., 1], 'Corrupted_im')
                    save_image(cor_all, 'Corrupted')
                    save_image(torch.abs(curr_target_frame - output), 'Error')
...
def load_model(checkpoint_file,args):
    ...
    # checkpoint_file = 'summary_after_optimization/summary/test/best_model.pt'
...
def train(args,order=None, init_trajs=[]):
    ...
    ####################################################################################
    # send_email(-1,0,0,0,0, "Staring Training:")
    eval_losses = []
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if len(eval_losses) >= 10 and all(loss == eval_losses[-1] for loss in eval_losses[-10:]):
            logger.info('Early stopping - loss has not improved in 10 epochs. Last loss: {eval_losses[-1]}')
            break

        # send_email(epoch=epoch, dev_loss=dev_loss, psnr_dev=psnr_dev, train_loss=train_loss, psnr_train=psnr_train)
...
def create_arg_parser():
    ...
    parser.add_argument('--data-path', type=pathlib.Path,
                      default='C:/Users/eszna/OneDrive - Technion/Wet_HW/Vision-Project/teamPilotData/raw', help='Path to the dataset')
    ...
    parser.add_argument('--fsim', default=False,action='store_true',help="calculate fsim values (advised to only use this over a trained model, not in training - computing fsim takes ~30 secs)")
    parser.add_argument('--vif', default=False, action='store_true',
    ...
    parser.add_argument('--initialization', type=str, default='cartesian',
    ...
    parser.add_argument('--tranformer_autoreg_method', action='store_true', default='traj', help='Train per shot or per trajectory.')
    parser.add_argument('--alternate-training', action='store_true', default=False,
                        help='Alternate training between subsampling and reconstruction model every 10 iterations')
    return parser
