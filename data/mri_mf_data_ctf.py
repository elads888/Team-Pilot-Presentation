        ...
        for fname in sorted(files):
            with h5py.File(fname, 'r') as data:
                ...
                if not 'aug.h5' in fname:
                    ...
                    if kspace.shape[2] < 30:
                        missing_dim = 30-kspace.shape[2]
                        missing_shape = list(kspace.shape)
                        missing_shape[2] = missing_dim
                        completion = np.repeat(np.expand_dims(kspace[:, :, -1, :, :], 2), missing_dim).reshape(missing_shape)

                        kspace = np.concatenate((kspace, completion), axis=2)
                    if kspace.shape[2] > 30:
                        kspace = np.concatenate(
                            (kspace[:,:,:29, :, :], np.expand_dims(np.mean(kspace[:,:, 31:, :, :], axis=2), axis=2)),
                            axis=2)
    ...
    def __getitem__(self, i):
        ...
        with h5py.File(fname, 'r') as data:
            if not 'aug.h5' in fname: #regular non-augmented file
                ...
                if kspace.shape[1] < 24:
                    kspace = np.concatenate((kspace,kspace[:,-1,:,:]),axis=1)
                if kspace.shape[1] > 24:
                    kspace = np.concatenate((kspace[:,:23,:,:],np.expand_dims(np.mean(kspace[:,25:,:,:],axis=1),axis=1)),axis=1)
                target = data['reconstruction_rss'][slice, start_frame:last_frame] if 'reconstruction_rss' in data else None
                #kspace = kspace.sum(axis = 1)/kspace.shape[1] #sum over coils

                ts = list(self.transform(kspace, target, data.attrs, fname, slice))
                ts[1] = np.expand_dims(ts[1],1)
                return ts
            else:
