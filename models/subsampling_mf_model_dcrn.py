...
from DRCN.DynamicRadCineMRI.network.reconstruction_network import NUFFTCascade as dcrn
from DRCN.DynamicRadCineMRI.network.nufft_operator import Dyn2DRadEncObj as RadEnc
from DRCN.DynamicRadCineMRI.network.xtyt_fft_unet import XTYTFFTCNN
...
class Subsampling_Model(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob, decimation_rate, res,
        ...
        self.unet = UnetModel(in_chans, out_chans, chans, num_pool_layers, drop_prob)
        n_enc_stages = 3
        n_convs_per_stage = 2
        n_filters = 6
        cnn_model = XTYTFFTCNN(n_ch=2,
				 n_enc_stages=n_enc_stages,
				 n_convs_per_stage=n_convs_per_stage,
				 n_filters=n_filters)
        self.reconstruction_model = dcrn(EncObj=None, CNN=cnn_model, nu=1,npcg=4,mode='pre_training', use_precon=False, learn_lambda=True)

    def forward(self, input):
        subsampled_input = self.subsampling(input)
        #subsampled_input = transforms.complex_abs(subsampled_input)
        Nt = input.shape[1]
        formatted_ktraj = self.get_trajectory().permute(0,2,1)
        subsampled_input = subsampled_input.permute(0,4,2,3,1).unsqueeze(0)
        density_comp = torch.ones((1,1,1,formatted_ktraj.shape[2],Nt)).to(self.device)
        self.reconstruction_model.EncObj = RadEnc(subsampled_input.shape,formatted_ktraj.repeat_interleave(Nt).view(*formatted_ktraj.shape,Nt),density_comp,None)
        output = transforms.complex_abs(self.reconstruction_model(subsampled_input.squeeze(0)).squeeze(0).permute(0,4,2,3,1))
        return output + transforms.complex_abs(subsampled_input.squeeze(0).permute(0,4,2,3,1))
