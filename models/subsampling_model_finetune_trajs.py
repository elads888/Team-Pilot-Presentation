    ...
    def forward(self, input):
        ...
        if self.interp_gap > 1:
            ...
            if len(self.x.shape) == 3:
                ...
                for frame in range(x_short.shape[0]*(1 + self.traj_finetune)):
                    
                    x_full = self.x[frame if frame<x_short.shape[0] else frame - x_short.shape[0]].reshape(-1, 2)
...
class Subsampling_Model_VST(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_layers, depth, drop_prob, decimation_rate, res,
                 trajectory_learning, initialization, n_shots, interp_gap, multiple_trajectories=False,
                 projection_iters=10e2, project=False, SNR=False, motion = False, device='cuda' if torch.cuda.is_available() else 'cpu', traj_finetune=False):
        super().__init__()
        self.device = device
        self.motion = motion
        if multiple_trajectories:
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device,
                                                 num_trajectories=in_chans,traj_finetune=traj_finetune)
        else:
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device)
        slice_num = 1
