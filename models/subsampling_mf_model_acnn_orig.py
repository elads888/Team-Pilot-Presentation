...
from ACNN.models.acnn import AcnnModel
    ...
    def initilaize_trajectories(self, trajectory_learning, initialization, n_shots, num_trajectories):
        ...
        if initialization == 'spiral':
            ...
            for i in range(n_shots):
                ...
                x[i, :, 1] = Ly * np.sin(theta * i)
            #noisy init
            # x = torch.zeros(num_trajectories, n_shots, sampel_per_shot, 2)
            # for j in range(num_trajectories):
            #     theta = np.pi / n_shots
            #     for i in range(n_shots):
            #         Lx = torch.arange(-144 / 2, 144 / 2, 144 / sampel_per_shot).float()
            #         Ly = torch.arange(-384 / 2, 384 / 2, 384 / sampel_per_shot).float()
            #         x[j,i, :, 0] = Lx * np.cos(theta * i)
            #         x[j,i, :, 1] = Ly * np.sin(theta * i)
            #         for k in range(sampel_per_shot):
            #             num = np.random.rand()
            #             if num > noise_precentage:
            #                 sign = np.random.randint(0,2)
            #                 if sign:
            #                     x[j, i, k, 1] -= np.random.rand()*noise_factor
            #                     x[j, i, k, 0] -= np.random.rand() * noise_factor
            #                 else:
            #                     x[j, i, k, 1] += np.random.rand()*noise_factor
            #                     x[j, i, k, 0] += np.random.rand() * noise_factor
        ...
        if initialization not in ['uniform', 'gaussian'] and num_trajectories is not None:
        ...
        if self.interp_gap > 1:
            ...
            if len(self.x.shape) == 3:
                ...
                if not self.groups_num:
                    ...
                    #groups
                    #11112222 case
                    group_size = x_short.shape[0]//self.groups_num
                    for group in range(self.groups_num):
                        traj_rep = self.curr_frame#torch.randint(0,group_size,(1,)).item() #update by which representative of the group
                        frame = group*group_size+traj_rep
                        # for orig_fr in range(group_size):
                        #     self.x.data[group*group_size+orig_fr] = self.x.data[frame]
                        x_full = self.x[frame].reshape(-1, 2)
                        for gr_frame in range(group_size):
                            curr_input = input[:, gr_frame + group_size*group].permute(0, 3, 1, 2)
                            sub_ksp = nufft(curr_input.unsqueeze(1), x_full, device=self.device)
                            output.append(nufft_adjoint(sub_ksp, x_full, curr_input.unsqueeze(1).shape, device=self.device))

                    output = torch.cat(output, dim=1)#.repeat_interleave(group_size,dim=1)

                    #update single frame case
                    #group_size = 1
                    # for group in range(8):
                    #     #traj_rep = self.curr_frame  # torch.randint(0,group_size,(1,)).item() #update by which representative of the group
                    #     frame = self.curr_frame #* group_size + traj_rep
                    #     # for orig_fr in range(group_size):
                    #     #     self.x.data[group*group_size+orig_fr] = self.x.data[frame]
                    #     x_full = self.x[frame].reshape(-1, 2)
                    #
                    #     curr_input = input[:, group].permute(0, 3, 1, 2)
                    #     sub_ksp = nufft(curr_input.unsqueeze(1), x_full, device=self.device)
                    #     output.append(
                    #         nufft_adjoint(sub_ksp, x_full, curr_input.unsqueeze(1).shape, device=self.device))
                    #
                    # output = torch.cat(output, dim=1)  # .repeat_interleave(group_size,dim=1)
                    ...
                    # group_size = x_short.shape[0]//self.groups_num
                    # for group in range(self.groups_num):
                    #     traj_rep = 0#torch.randint(0,group_size,(1,)).item() #update by which representative of the group
                    #     frame = group#group*group_size+traj_rep
                    #     for orig_fr in range(group_size):
                    #         self.x.data[group+self.groups_num*orig_fr] = self.x.data[frame]
                    #     x_full = self.x[frame].reshape(-1, 2)
                    #     curr_input = input[:, frame].permute(0, 3, 1, 2)
                    #     sub_ksp = nufft(curr_input.unsqueeze(1), x_full, device=self.device)
                    #     output.append(nufft_adjoint(sub_ksp, x_full, curr_input.unsqueeze(1).shape, device=self.device))
                    #
                    # output = torch.cat(output, dim=1).repeat(1,4,1,1,1)
    ...
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob, decimation_rate, res,
        ...
        if multiple_trajectories:
            ...
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device)
        slice_num = 1
        self.reconstruction_model = AcnnModel(8 * 2 * slice_num, 8 * 2 * slice_num, 64, 4, 0, slice_num, False)#UnetModel(in_chans, out_chans, chans, num_pool_layers, drop_prob)
