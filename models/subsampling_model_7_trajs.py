...
class Subsampling_Layer(nn.Module):
    ...
    def forward(self, input):
        ...
        for frame in range(8):
            ...
            for i in range(16):
                ax.plot(self.x[frame%7, i, :, 0].cpu().detach(), self.x[frame%7, i, :, 1].cpu().detach())
        ...
        for frame in range(8):
            ...
            for i in range(16):
                ...
                ax.plot(range(8),torch.cat((self.x[:, i, 513//(frame+2), 0],self.x[0, i, 513//(frame+2), 0].unsqueeze(0)),0).cpu().detach())
                ax.axis([-8, 8, -8, 8])
                break
        ...
        for frame in range(8):
            ax = fig.add_subplot(2, 4, frame + 1)
            for i in range(16):
                ax.plot(range(8),torch.cat((self.x[:, i, 513//(frame+2), 1],self.x[0, i, 513//(frame+2), 1].unsqueeze(0)),0).cpu().detach())
                ax.axis([-8, 8, -8, 8])
                break
        ...
        if self.interp_gap > 1:
            ...
            if len(self.x.shape) == 3:
                ...
                for frame in range(x_short.shape[0]+1):
                    x_full = self.x[frame%x_short.shape[0]].reshape(-1, 2) # last frame is first for periodicity
