...
from CTFNet.kt_Dynamic_MRI_Reconstruction.network.CTFNet import CTFNet_model
...
class Subsampling_Layer(nn.Module):
    ...
    def forward(self, input):
        ...
        return output.permute(0, 1, 3, 4, 2), sub_ksp
...
class Subsampling_Model(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob, decimation_rate, res,
                 ...
                 projection_iters=10e2, project=False, SNR=False, motion = False, device='cuda', ctf = False):
        super().__init__()
        self.device = device
        self.motion = motion
        self.ctf = ctf
        if multiple_trajectories:
            ...
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device)
        self.reconstruction_model = CTFNet_model(chans,0.1,0.1,['2'],['1']) if self.ctf else UnetModel(in_chans, out_chans, chans, num_pool_layers, drop_prob)

    def forward(self, input):
        subsampled_input, sub_ksp = self.subsampling(input)
        subsampled_input = transforms.complex_abs(subsampled_input)
        x_und = subsampled_input.permute(2, 0, 3, 4, 1)  # n_seq, bs, width, height, n_ch
        k_und = sub_ksp.permute(2, 0, 3, 4, 5, 1)  # n_seq, bs, n_coil, width, height, n_ch
        masks = self.get_trajectory().permute(2, 0, 3, 4, 5, 1)
        x_smaps = torch.ones_like(x_und)
        output = self.reconstruction_model(x_und,k_und,masks,x_smaps) if self.ctf else self.reconstruction_model(subsampled_input)
        return output + subsampled_input
