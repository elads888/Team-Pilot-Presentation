...
from MRITramsformer.model import BasicTransformerTraj, BasicTransformerShot, AutoTrajPredictorAllFrames
from MRITramsformer.unet1 import UNetEncoder
from MRITramsformer.unet2 import UNetEncoder as Unet2
from MRITramsformer.unet3 import UNetEncoder as Unet3
...
class Subsampling_Layer(nn.Module):
    ...
    def __init__(self, decimation_rate, res, trajectory_learning, initialization, n_shots, interp_gap, \
                 projection_iterations=10e2, project=False, SNR=False, device='cuda' if torch.cuda.is_available() else 'cpu', num_trajectories=None, traj_finetune=False, batch_size=8):
        super().__init__()
        ...
        #self.unet_enc = UNetEncoder(in_channels=batch_size*2)
        #self.unet_enc2 = Unet2()
        #self.unet_enc3 = Unet3()

        checkpoint = torch.load("MRITramsformer/unet_encoder_pretrained.pth",
                                map_location="cuda" if torch.cuda.is_available() else "cpu")
        unet_encoder = Unet3(in_channels=2, base_channels=4)
        unet_encoder.load_state_dict(checkpoint)
        unet_encoder.to("cuda" if torch.cuda.is_available() else "cpu")
        unet_encoder.eval()
        self.unet_enc3 = unet_encoder
        self.auto_traj_pred_all_frames = AutoTrajPredictorAllFrames(device, batch_size=batch_size)
    ...
    def forward(self, input):
        ...
        if self.interp_gap > 1:
            ...
            # elif len(self.x.shape) == 4 or True:
            if len(Subsampling_Layer.downsampled_frames) == 8:
                del Subsampling_Layer.predictions  # 🟢 Free accumulated memory
                Subsampling_Layer.predictions = []
                Subsampling_Layer.downsampled_frames = []
                #torch.cuda.empty_cache()  # 🟢 Clear GPU cache

            #curr_frame = len(Subsampling_Layer.predictions)+1
            if len(Subsampling_Layer.downsampled_frames) == 0:
                with torch.no_grad():
                # taking only first because of the repeated trajectory
                    Subsampling_Layer.downsampled_frames = self.unet_enc3(input).unsqueeze(0)
            else:
                with torch.no_grad():
                    Subsampling_Layer.downsampled_frames = torch.cat((Subsampling_Layer.downsampled_frames, self.unet_enc3(input).unsqueeze(0)), dim=0)

            self.x = self.auto_traj_pred_all_frames(Subsampling_Layer.downsampled_frames).permute(1, 2, 3, 0)

            #Store prediction
            if len(Subsampling_Layer.predictions) == 0:
                Subsampling_Layer.predictions = self.x.to('cpu').detach()
            else:
                Subsampling_Layer.predictions = torch.cat((Subsampling_Layer.predictions, self.x.to('cpu').detach()), dim=0)
    ...
    def get_all_trajectories(self):
        return Subsampling_Layer.predictions
...
class Subsampling_Model(nn.Module):
    ...
    def get_trajectories(self):
        return self.subsampling.get_all_trajectories()
...
class FeatureVSTBInterleaved(nn.Module):
    def __init__(self, in_chans, layer_num,vst_depth, feature_num):
        ...
        for i in range(len(features)-1):
            layers.append(nn.Conv3d(features[i],features[i+1],(3,3,3),padding="same"))
            layers += [VSTB(dim=features[i+1],depth=vst_depth,num_heads=1,window_size=(2,4,4))]*layer_num
            #layers += [nn.ReLU()]
...
class Subsampling_Model_VST(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_layers, depth, drop_prob, decimation_rate, res,
                 trajectory_learning, initialization, n_shots, interp_gap, multiple_trajectories=False,
                 projection_iters=10e2, project=False, SNR=False, motion = False, device='cuda' if torch.cuda.is_available() else 'cpu', batch_size=8):
        super().__init__()
        self.device = device
        self.motion = motion
        if multiple_trajectories:
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device,
                                                 num_trajectories=in_chans, batch_size=batch_size)
        else:
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device, batch_size=batch_size)
        slice_num = 1
