...
from DDoS.models.ThisNewNet import ThisNewNet as DDoS_Net
...
class Subsampling_Model(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob, decimation_rate, res,
        ...
        if multiple_trajectories:
            ...
            self.subsampling = Subsampling_Layer(decimation_rate, res, trajectory_learning, initialization, n_shots,
                                                 interp_gap, projection_iters, project, SNR, device=device)
        self.reconstruction_model = DDoS_Net(scale_factor=(1,1,1),in_channels=1,n_classes=1,num_features=32)#UnetModel(in_chans, out_chans, chans, num_pool_layers, drop_prob)

    def forward(self, input):
        subsampled_input = self.subsampling(input)
        subsampled_input = transforms.complex_abs(subsampled_input)
        #output = self.reconstruction_model(subsampled_input.permute(1,0,2,3).unsqueeze(0))
        output = self.reconstruction_model(subsampled_input.unsqueeze(1)).squeeze(1)
