...
class UNetEncoder(nn.Module):
    def __init__(self, in_channels=2, base_channels=4):
        super().__init__()
        ...
        self.enc6 = self.conv_block(base_channels * 8, base_channels * 8)

        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.pool3 = nn.MaxPool2d(kernel_size=3, stride=3)
        self.finalPool = nn.MaxPool1d(kernel_size=6, stride=6)
    ...
    def forward(self, x):
        ...
        enc6 = self.enc6(self.pool3(enc5))  # (batch_size, base_channels*8, 8, 3)
        batch_size = x.size()[0]
        final_ebdim = enc6.size()[1]
        enc6_1 = enc6.reshape(batch_size, final_ebdim, -1) # (batch_size, base_channels*8, 24)
        bottleneck = self.finalPool(enc6_1)   # (batch_size, base_channels*8, 4)

        #batch_size = bottleneck.size()[0]
        #out_channels = bottleneck.size()[1]
        #bottleneck = bottleneck.permute(0,3,1,2).reshape(batch_size*3, out_channels, 8)
        ...
        return bottleneck.reshape(batch_size, -1)